...
from .padding import pad_same, pad_same_3d, get_padding_value
...
class AvgPool3dSame(nn.AvgPool3d):
    ...
    def forward(self, x):
        x = pad_same_3d(x, self.kernel_size, self.stride)
        return F.avg_pool3d(
            x, self.kernel_size, self.stride, self.padding, self.ceil_mode, self.count_include_pad)
...
def max_pool3d_same(
        x, kernel_size: List[int], stride: List[int], padding: List[int] = (0, 0, 0),
        dilation: List[int] = (1, 1, 1), ceil_mode: bool = False):
    x = pad_same(x, kernel_size, stride, value=-float('inf'))
...
class MaxPool3dSame(nn.MaxPool3d):
    ...
    def forward(self, x):
        x = pad_same_3d(x, self.kernel_size, self.stride, value=-float('inf'))
