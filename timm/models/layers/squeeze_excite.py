...
from .custom import select_conv
...
class SEModule(nn.Module):
    ...
    Additions include:
        ...
        * reduction channels can be specified directly by arg (if rd_channels
                                                               is set)
...
class ChannelSELayerNd(nn.Module):
    ...
    def __init__(self, num_channels, reduction_ratio=16):
        ...
        :param reduction_ratio: By how much should the num_channels should be
        reduced
        """
...
class SpatialSELayerNd(nn.Module):
    ...
    def __init__(self, in_shape):
        ...
        self.conv = select_conv(in_shape, ch_out=1, kernel_size=1, stride=1,
                                padding=0)
        self.sigmoid = nn.Sigmoid()
...
class ChannelSpatialSELayerNd(nn.Module):
    """
       3D extension of concurrent spatial and channel squeeze & excitation:
           *Roy et al., Concurrent Spatial and Channel Squeeze & Excitation in
           Fully Convolutional Networks, arXiv:1803.02579*
       """

    def __init__(self, in_shape, reduction_ratio=16):
        """
        :param num_channels: No of input channels
        :param reduction_ratio: By how much should the num_channels should be
        reduced
        ...
        self.cSE = ChannelSELayerNd(in_shape[1], reduction_ratio)
        self.sSE = SpatialSELayerNd(in_shape)
...
class SEModuleNd(ChannelSpatialSELayerNd):
    def __init__(self, in_shape, rd_ratio=16):
        # `channels` only here for API consistency
        super(SEModuleNd, self).__init__(in_shape, rd_ratio)
